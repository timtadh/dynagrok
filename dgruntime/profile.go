...
func (p *Profile) SerializeInProfs(fout io.Writer) {
	for fname, profs := range p.Inputs {
		fmt.Fprint(fout, FuncProfile{fname, profs}.Serialize())
	}
}

func (p *Profile) SerializeOutProfs(fout io.Writer) {
